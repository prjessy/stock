...
class AutoSellWatcher:
    def __init__(self, registry, interval: float = 15.0) -> None:
        ...
        self._done: dict[str, set] = {}  # {거래일: {종목}} — 발동 완료

    def start(self) -> None:
        if self._thread is not None:
            return
        self._thread = threading.Thread(
            target=self._loop, daemon=True, name="autosell-watch"
        )
        self._thread.start()
    ...
    def _sell(self, client: OrderClient, sym: str, h: dict, rule: dict,
              cur: float, reason: str) -> None:
        qty = int(h["qty"])
        cap = int(rule.get("max_qty") or 1)
        name = h.get("name") or sym
        res = client.place_order(sym, "sell", qty, price=cur, cap=cap)
        sold = min(qty, cap)
        if res.get("ok"):
            notify_all(
                "🤖 자동 매도 실행",
