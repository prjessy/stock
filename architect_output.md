...
## 10. 실시간 데이터 로드맵 (향후 업데이트 예정)
- **현재**: 무료 소스(국내 FinanceDataReader, 미국 yfinance) 사용 → **준실시간(약 15분 지연)**, 대시보드 30초 자동 갱신. UI에 명시.
- **향후**: **한국투자증권(KIS) OpenAPI 실시간**으로 교체 예정.
  - 국내(005930/000660) 실시간 시세(REST/웹소켓) 지원. (계좌·앱키/앱시크릿 필요)
  - 미국 종목은 KIS 해외주식 시세 또는 별도 실시간 소스 검토.
- **교체 방식(영향 최소화)**: 데이터는 `app/datasources/`의 `PriceSource` 인터페이스 + `registry.py`로 분리되어 있어,
  - `kis_price.py`(신규 구현체)를 추가하고 `registry.py`에서 매핑만 교체하면 됨.
  - **대시보드 UI / 알림 로직 / 임계값 엔진은 변경 불필요.**
- **영향 참고**: 실시간 전환 시 ±3% 알림 지연도 함께 해소됨(현재는 무료 소스라 알림도 ~15분 지연).
- 적용 시점: 무료 버전 안정화 후, 필요 시 진행.
